...
export class BotWeatherCommand extends BotSlashCommand {
    ...
    execute(interaction : any) {
        ...
        fetch('http://api.openweathermap.org/data/2.5/weather?zip=' + zipcode + ',us&appid=' + WEATHER_API_KEY + '&units=' + (prefersCelcius ? 'metric' : 'imperial'))
        ...
        .catch((error : any) => {
            console.error(error);
            interaction.reply('Error getting weather');
        });
    }
}